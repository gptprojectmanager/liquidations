...
"""
    ...
    def test_load_csv_returns_timezone_naive_timestamps(self, temp_dir):
        """Test that timestamps are timezone-naive for consistent date comparisons.

        Regression test: Date filtering failed with timezone comparison error.
        DuckDB to_timestamp() returns timezone-aware, but Python datetime
        comparisons require timezone-naive for consistency.
        """
        csv_file = temp_dir / "test.csv"
        csv_content = """timestamp,symbol,sumOpenInterest,sumOpenInterestValue,countTopTraderLongShortRatio
1698537600000,BTCUSDT,123456.78,8123456789.12,1.23
"""
        ...
        # Assert: Timestamps must be timezone-naive (tz is None)
        assert df["timestamp"].dt.tz is None, (
            "Timestamps must be timezone-naive for date filtering compatibility"
        )
