...
- **FR-004**: System MUST identify high-density zones (top 20% by liquidation volume) in both datasets
- **FR-005**: System MUST calculate hit_rate as percentage of Coinglass major zones that match our predictions (within ±1% price tolerance)
...
3. Zone Classification
   - Current price from screenshot title or API
   - Zones < current_price → long liquidations
   - Zones > current_price → short liquidations

4. High-Density Filtering
   - Visual inspection: Darker regions = higher density
   - Heuristic: Top 20% of zones by pixel darkness
   - Alternative: Use all extracted levels (simpler, no image analysis)
```
